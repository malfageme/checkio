def choose_good_gift(current_gift, gifts_in_bag, gift_number):
    ...
    if gift_number == 1:
        best = 0
    if (gift_number*100.0)/gifts_in_bag < 37:
        best = max(best,current_gift)
    else:
        #print "Above 37%% --> gift_number: %d" % (gift_number)
        if current_gift > best:
            #print "FOUND --> best: %f gift_number: %d current: %f" % (float(best), gift_number, float(current_gift))
            return True
    return False
